  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            className="w-[350px] shadow-sm max-w-full relative rounded-md border flex-shrink-0 px-8 py-4 md:w-[450px]"
            key={item.name}
          >
            <blockquote>
              <span
                className={cn(
                  " relative text-sm leading-[1.6] font-normal",
                  lusitana.className
                )}
              >
                {item.quote}
              ...
              <div className="relative mt-6 flex flex-row items-center">
                <span className="flex flex-col gap-1">
                  <span className=" text-sm leading-[1.6] ">{item.name}</span>
